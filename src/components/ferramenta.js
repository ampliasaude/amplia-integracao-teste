...
function adjustObservableWidth(visRef,router) {
    ...
    mainNotebook.redefine("lang",window.location.href.indexOf('/en/')>=0?'en':'pt');
    ...
    function handleRouteChange(url) {
      mainNotebook.redefine("lang",url.indexOf('/en/')>=0?'en':'pt');
    }
...
export function MapaInternal() {
  ...
  useEffect(() => {
    ...
    mainNotebook = main;
    let free = adjustObservableWidth(mainWindowRef,router);
    return () => {runtime.dispose(); free(); };
  }, []);
...
function NascidosVivosInternal() {
  ...
  useEffect(() => {
    ...
    mainNotebook = main;
    let free = adjustObservableWidth(visRef,router);
    return () => { runtime.dispose(); free(); };
  }, []);
  ...
  return (
    <div className="menu-vis">
      <div className="bt bt-mapa" onClick={mapaClick}>
        ...
        {(router.query.locale == "pt" ? "Escolha municípios" : "Select Municipalities")}
      </div>
